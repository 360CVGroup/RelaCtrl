...
class InternalData(Dataset):
    def __init__(self,
        ...
        for json_file in image_list_json:
            ...
            self.vae_feat_samples.extend([os.path.join(self.root, f'img_vae_features/{resolution}resolution/noflip', '_'.join(item['path'].rsplit('/', 1)).replace('.png', '.npy')) for item in meta_data_clean])
            # self.vae_feat_samples.extend([os.path.join(self.root, f'img_vae_features_{resolution}resolution/noflip', item['path'].rsplit('/', 1).replace('.png', '.npy')) for item in meta_data_clean])
